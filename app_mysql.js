...
app.set('views', './views_mysql');//views 폴더는 views_files
...
app.post('/topic/:id/edit', function(req, res){//router 설정하고 그 안에 controller내용 넣는다.
  //controller
  ...
  conn.query(sql,[title,description,author,id],function(err,rows,fields){
	...
	//res.send(rows);//여기에서 insertId값을 가져올 수 있음. 무슨 값이지?
	res.redirect('/topic/'+id);//위에서 insertId값을 가져옴.
  });
});

app.get('/topic/:id/delete', function(req, res){//router 설정하고 그 안에 controller내용 넣는다.
  //controller
...
app.post('/topic/:id/delete', function(req, res){//router 설정하고 그 안에 controller내용 넣는다.
  //controller
    var id=req.params.id;
  	var sql='delete from topic where id=?';
	conn.query(sql,[id],function(err,rows,fields){
...
app.post('/topic/add', function(req, res){//router 설정하고 그 안에 controller내용 넣는다.
  //controller
  var title = req.body.title; //cf. get req.query.title
  var description = req.body.description;
  var author = req.body.author;
  ...
  conn.query(sql,[title,description,author],function(err,rows,fields){
	...
	//res.send(rows);//여기에서 insertId값을 가져올 수 있음. 무슨 값이지?
	res.redirect('/topic/'+rows.insertId);//위에서 insertId값을 가져옴.
  });
});
